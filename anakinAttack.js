...
if(anakinHealth <= obiWanAttack - anakinDefense){
  //same thing as (anakinhealth= ankinhelth -obiwanattack -anakin defense
  console.log(`Anankin has lost all limbs`)
} else {
  anakinHealth -= obiWanAttack - anakinDefense
  console.log(`Anakin's health is now ${anakinHealth}`)
 }

console.log(`Anakin uses Drugs`)
...
while(anakinHealth > 0){
 anakinHealth -= obiWanAttack - anakinDefense 
  console.log(`Anakin's health is now ${anakinHealth}`)
  if(anakinHealth <= 0){
    console.log(`Anakin is now a limbless gimp`)
  }
}